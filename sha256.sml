...
structure SHA256
   ...
   struct
      ...
      *)
      ...
      val hshInit : W.word Vector.vector =
         Vector.fromList
      ...
      fun hash s =
         let
            val hsh : W.word A.array = A.array (8, 0w0)
            ...
            fun dostep i (a, b, c, d, e, f, g, h) =
               if i >= 64 then
                  (
                  A.update (hsh, 0, W.+ (A.sub (hsh, 0), a));
                  A.update (hsh, 1, W.+ (A.sub (hsh, 1), b));
                  A.update (hsh, 2, W.+ (A.sub (hsh, 2), c));
                  A.update (hsh, 3, W.+ (A.sub (hsh, 3), d));
                  A.update (hsh, 4, W.+ (A.sub (hsh, 4), e));
                  A.update (hsh, 5, W.+ (A.sub (hsh, 5), f));
                  A.update (hsh, 6, W.+ (A.sub (hsh, 6), g));
                  A.update (hsh, 7, W.+ (A.sub (hsh, 7), h))
                  )
               else
                  let
                     open W
            ...
            val () = doHash (chunk32 0 s)
         in
            Bytestring.concat
            (Array.foldr (fn (w, l) => ConvertWord.word32ToBytesB w :: l) [] hsh)
         end

      fun hashBytes str = hash (Stream.fromTable Bytestring.sub str 0)
      fun hashString str = hash (Stream.fromTable (fn (str', i) => Word8.fromInt (Char.ord (String.sub (str', i)))) str 0)
